...
$internalFields = array(
    ...
    "frc-captcha-solution", "Frc-captcha-version"
);
...
function getTopText ($replyTo) {
    $message = "You received a new message sent via the contact form on schwamm-drueber.ch.";
    $message = strlen($message) > 0 ? $message : "You received a new message from __REPLYTO__ sent via the contact form on schwamm-drueber.ch.";
    $message = str_replace("__REPLYTO__", $replyTo, $message);
...
function getMessageToSend ($replyTo, $boundary, $skipFieldIfEmpty = true) {
    ...
    $internalFieldsLC = array_map("toLower", $internalFields);

    foreach($_POST as $field => $value) {
        if (in_array(toLower($field), $internalFieldsLC)) {
            // Just skip
    ...
    $message .= chunk_split(base64_encode(getMessageHTML($topText, $arrMessage)));
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "schwamm-drueber.ch"]) . ">",
        "Date: ".date("r (T)"),
        "X_Mailer: PHP/" . phpversion(),
        "X-G1i-WSB-Sendmail: 124.0.30",
    );
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "schwamm-drueber.ch") {
...
function processMail() {
    ...
    if (empty($response)) {
        $contactFormEmails = json_decode('{"6FB22108-E1C9-4BC9-AA99-70A95C085133":"sos@schwamm-drueber.ch"}', true);
        ...
        if (empty($response)) {
            ...
            // NOTE: Boundary should be used only for multipart
            $boundary = md5(time());
